...
object Irish1901App {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val stringRdd = sess.sparkContext.textFile("./data/data/census/ireland/1901/1901-[1-4]/census-[0-9]*.csv")
    stringRdd.cache()
    ...
    val basePipe = IrishRecipeUtils.buildBase(1901)
    //buildMain
    val iniPipe = IrishRecipeUtils.buildInitials(1901) //buildMain
    ...
    val resPipe = IrishRecipeUtils.buildFractionPipeline(1901, extracted.columns.filter(f => f.startsWith("appr")))
    val resModel = resPipe.fit(extracted)
    val result = resModel.transform(extracted)

    val writer = new CsvWriter("./data/results/ire_1901_baby_names.csv", "UTF-8", false)
