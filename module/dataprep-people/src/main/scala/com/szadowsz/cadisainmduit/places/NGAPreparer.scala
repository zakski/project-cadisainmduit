...
import com.szadowsz.spark.ml.LocalDataframeIO
...
object NGAPreparer extends LocalDataframeIO {
  ...
  def getOtherCountries(sess : SparkSession):DataFrame = {
    ...
    val cutdownDfs = placesDfs.map { placesDf =>
      ...
      val t = new CsvColumnExtractor("X").setInputCol("fields").setOutputCols(ngaSchema).setSize(ngaSchema.length).setDelimiter('\t')
