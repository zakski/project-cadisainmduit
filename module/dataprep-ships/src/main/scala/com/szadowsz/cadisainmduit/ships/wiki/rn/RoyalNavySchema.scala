...
object RoyalNavySchema {
  ...
  private[wiki] def shapeFinalData(dfShip: DataFrame): DataFrame = {
    val typeData = new CsvReader("./data/tmp/uboat/uboatInfo.csv")
    val typeSet = typeData.readAll().filter(row => row(4) == "Commonwealth").map(s => (s.head, s(1))).toSet
