...
object RoyalNavyPreparer extends LocalDataframeIO {
  ...
  private def buildShipPipe(): Lineage = {
    ...
    pipe.addStage(classOf[CsvColumnExtractor], "inputCol" -> "fields", "outputCols" -> RoyalNavySchema.classSchema, "size" -> RoyalNavySchema.classSchema.length)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dfShip = extractFile(sess, new File("./data/data/web/shipsRN.csv"), false)
    ...
    writeDF(candtDF, "./data/tmp/rn/rnInfo.csv", "UTF-8", (s: Seq[String]) => true, finalOrd)
  }
}