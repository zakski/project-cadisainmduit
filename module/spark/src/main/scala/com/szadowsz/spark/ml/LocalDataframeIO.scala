...
import com.szadowsz.spark.ml.feature.CsvColumnExtractor
...
trait LocalDataframeIO {
  ...
  protected def extractSchema(f : File): Array[String] = {
    val r = new FReader(f.getAbsolutePath)
    val schema = r.readLine()
    r.close()
    schema.split(",")
  }
  ...
  protected def extractFile(sess: SparkSession, f: File, dropFirst: Boolean, readSchema: Boolean = false, delimiter: Char = ','): DataFrame = {
    ...
    if (readSchema && dropFirst) {
      val t = new CsvColumnExtractor(s"${f.getName} Loader").setInputCol("fields").setOutputCols(fields).setSize(fields.length).setDelimiter(delimiter)
      df = t.transform(df)
    }
