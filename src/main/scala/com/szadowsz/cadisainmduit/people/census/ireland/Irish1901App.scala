...
object Irish1901App {
  ...
  def main(args: Array[String]): Unit = {
    ...
    DeleteUtil.delete(new File("./data/census/ireland/1901/"))
    val files = FileFinder.search("./archives/data/census/ireland/1901", Some(new ExtensionFilter(".zip", false)))
    files.foreach(f => ZipperUtil.unzip(f.getAbsolutePath,"./data/census/ireland/1901"))
    ...
    val basePipe = RecipeUtils.buildBase(1901)
    //buildMain
    val iniPipe = RecipeUtils.buildInitials(1901) //buildMain
    ...
    val resPipe = RecipeUtils.buildFractionPipeline(1901, extracted.columns.filter(f => f.startsWith("appr")))
    val resModel = resPipe.fit(extracted)
    val result = resModel.transform(extracted)

    val writer = new CsvWriter("./data/census/ire_1901_baby_names.csv", "UTF-8", false)
