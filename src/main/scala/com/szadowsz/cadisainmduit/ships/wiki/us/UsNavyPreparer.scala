...
object UsNavyPreparer {
  ...
  private def buildShipPipe(): Lineage = {
    ...
    pipe.addStage(classOf[CsvTransformer], "inputCol" -> "fields", "outputCols" -> UsNavySchema.classSchema, "size" -> UsNavySchema.classSchema.length)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dfShip = extractFile(sess, new File("./archives/data/web/shipsUSA.csv"), false)
    ...
    writeDF(candtDF, "./data/web/usn/usnInfo.csv", "UTF-8", (s: Seq[String]) => true, finalOrd)
  }
}