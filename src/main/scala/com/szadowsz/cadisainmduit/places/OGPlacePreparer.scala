...
object OGPlacePreparer extends LocalDataframeIO {
  ...
  def getUK(sess : SparkSession): DataFrame = {
    ...
    pipe.addStage(classOf[CsvTransformer], "inputCol" -> "fields", "outputCols" -> placeUKSchema, "size" -> placeUKSchema.length)
    val m = pipe.fit(df)
  ...
  def main(args: Array[String]): Unit = {
    ...
    pipe.addStage(classOf[CsvTransformer], "inputCol" -> "fields", "outputCols" -> openGovSchema, "size" -> openGovSchema.length)
