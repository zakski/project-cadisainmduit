...
import com.szadowsz.grainne.data.Language
...
/**
  ...
  */
class LanguageCell extends CellProcessorAdaptor with Serializable {
  ...
  private def matchLangs(desc: String): List[Language] = {
    ...
    val results = candidates.filter( can => can._1._2 > 0.66 || can._2._2 > 0.8).map(can => if (can._1._2 > can._2._2) can._1 else can._2)
